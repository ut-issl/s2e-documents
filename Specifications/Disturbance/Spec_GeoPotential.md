...
1. Functions
   ...
   - The `GeoPotential` class calculates the gravity acceleration of the earth with [EGM96](https://cddis.nasa.gov/926/egm96/) geo-potential model.
   - 
2. Related files
...
3. How to use
   ...
   - Select `ENABLE` and `degree` of calculation
     ...
     - **NOTE**: The calculation time is related with the selected degree.
...
1. Read coefficients
   ...
   3. algorithm
      ...
      - The file format of `egm96_to360.ascii` is `n,m,Cnm,Snm,sigmaCnm,sigmaSnm` in a line with space  delimiter. In this calculation, the `sigmaCnm` and `sigmaSnm` are not used.

      - Total number of read line is defined as following equation
        ```math
   ...
   4. note
      ...
      - **When reading file is failed**, the maximum degree is set to zero, and simple Kepler calculation will be executed.
...
For unnormalized algorithm, see chapter 3.2.4 of [Satellite Orbits](https://www.springer.com/jp/book/9783540672807).
...
The recursion calculation of V and W can changed to normalized version as follows
...
For unnormalized algorithm, See  chapter 3.2.5 of [Satellite Orbits](https://www.springer.com/jp/book/9783540672807). 
...
```
   ...
   4. note
      ...
      - To accelerate the calculation, double `for` loop of acceleration calculation and the recursion loop need to be integrated in future.
...
1. Calculation accuracy 
   1. overview
      ...
      - The satellite position in ECEF frame calculated by S2E is inputted to the  `CalcAccelerationECEF` and `gravitysphericalharmonic( r, 'EGM96',360, 'Error' );` function of Matlab.
   ...
   3. results
      ...
      - Difference between `CalcAccelerationECEF` output when `degree=0=1` and Matlab's output (`degree = 360`) 
        - You can see large error since `CalcAccelerationECEF` doesn't care high-order gravity potential.
   ...
   4. Note
         ...
         - To check the accuracy of the calculation, resolution of log output should be larger than 10.
           - In this case, I chose the resolution as 15.
2. Calculation speed
   1. overview

      - I've checked the relationship between the degree and the calculation speed.
